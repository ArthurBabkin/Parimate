...
class FaceAnalysis:
    ...
    def extract_embedding(self, image: np.ndarray) -> list[float]:
        ...
        if len(embeddings) != 1:
            raise ValueError(
                "There should be exactly one face in the reference image.")
    ...
    def verify_face(self, image: np.ndarray, reference: list[float]) -> bool:
        ...
        result = DeepFace.verify(
            image, reference, detector_backend=self.detector,
            model_name=self.model_name, silent=True, enforce_detection=False
        )
...
def convert_base64_to_np(img_b64: str) -> np.ndarray:
    ...
    """
    return cv2.imdecode(
        np.frombuffer(base64.b64decode(img_b64), dtype=np.uint8), -1)
